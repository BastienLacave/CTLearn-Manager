...
class Combinator2000(DL2DataProcessor):
    def __init__(
        self, direction_processor, energy_processor, type_processor, gammaness_cut=0.9
    ):
        ...
        self.dl2s = [
            join(
                self.dl2s,
                self.type_dl2[
                    ["obs_id", "event_id", self.type_processor.gammaness_key]
                ],
                keys=["obs_id", "event_id"],
                join_type="left",
            )
        ...
        for DL2 in tqdm(self.dl2s, desc="Computing sky positions"):
            ...
            reco_temp = SkyCoord(
                alt=DL2[self.reco_alt_key], az=DL2[self.reco_az_key], frame=frame
            )  # , obstime=DL2["time"])
            pointing_temp = SkyCoord(
                alt=DL2[self.pointing_alt_key],
                az=DL2[self.pointing_az_key],
                frame=frame,
            )  # , obstime=dl2["time"])
